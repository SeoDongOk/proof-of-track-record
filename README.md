...
이건 [모 저장소의 백테스트에서 실제로 겪은 문제](https://github.com/SeoDongOk/Algorithmic_Trading_YL)다.
연 23.63% 가 생존 편향 제거 후 14.48% 가 됐고, 초과수익 p 값은 0.566 이었다.
**백테스트는 마음에 들 때까지 다시 돌릴 수 있고, 그 흔적이 남지 않는다.**
사전 커밋은 그걸 불가능하게 만든다.
